...
function getUsersData(){
    fetch("/inspiracioncomfama/usuarios", {
        method:"GET",
        headers:{
    ...
    .then(function(response){
        ...
        response.data.forEach(function(e,i){
            fetch("/inspiracioncomfama/school/"+e.document,{
                method:"GET",
                headers:{
            ...
            .then(function(response){
                if(response.ok){
                    ...
                    <tr onclick="getUserInfo('${e._id}','${e.name}','${e.email}','${e.parentName}','${e.number}','${e.document}','${response.name}');" data-toggle="modal" data-target="#userInfoModal">
                        ...
                        <td>${e.date?e.date:"No Disponible"}</td>
                    </tr>
                `);
                }
            })
...
function getSchool(doc){
    var school='';
    fetch("/inspiracioncomfama/school/"+doc,{
        method:"GET",
        headers:{
            "Content-Type":"applicaiont/json"
        }
...
function getUserInfo(id, user, email, parent, number, doc, name){
    ...
    `);
    fetch("/inspiracioncomfama/usuario/"+id,{
        method:"GET",
        headers:{
            "Content-Type":"application/json"
        }
